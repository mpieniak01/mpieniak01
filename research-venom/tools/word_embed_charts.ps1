...
param(
    ...
    [string]$WordMapCsv = "",
    [string]$StagingRoot = "C:\temp\visualization_word_embed_staging",
...
function Resolve-DefaultPaths {
    ...
    if (-not $OutRunJson) { $OutRunJson = Join-Path $base "products_light\visualization\word_embed_run_v04.json" }
    if (-not $OutRunMd) { $OutRunMd = Join-Path $base "products_light\visualization\word_embed_run_v04.md" }
    return @{
...
$md = @()
$reportLabel = [System.IO.Path]::GetFileNameWithoutExtension($OutRunMd)
if (-not $reportLabel) { $reportLabel = "word_embed_run" }
$md += "# Word Embed Run $reportLabel"
$md += ""
