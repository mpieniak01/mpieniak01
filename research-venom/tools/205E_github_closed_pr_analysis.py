...
def markdown_report(
    ...
    lines.append(
        "2. Skrypt: `docs_dev/_to_do/205_artifacts/tools/github_closed_pr_analysis.py` z paginacją (`per_page=100`) i retry/rate-limit guard."
    )
    lines.append(
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--output-json",
        default="docs_dev/_to_do/205_artifacts/205/analysis/205_analiza_pr.json",
    )
    p.add_argument(
        "--output-md",
        default="docs_dev/_to_do/205_artifacts/205/analysis/205_analiza_pr.md",
    )
    return p.parse_args()
