...
function Resolve-DefaultPaths {
    ...
    if (-not $PatternsDoc) { $PatternsDoc = Join-Path $repoRoot "docs\EXCEL_ADVANCED_PATTERNS.md" }
    return @{
...
try {
    ...
    foreach ($chartCfg in @($chartSpec.charts)) {
        ...
        if ($isTypeB) {
            $targetSheet = [string]$chartCfg.sheet
            if (-not $targetSheet) { $targetSheet = [string]$chartCfg.source_sheet }
            $sheetObj = $null
            try { $sheetObj = $wb.Worksheets.Item($targetSheet) } catch { $sheetObj = $null }
            $co = $null
            if ($sheetObj) {
                try { $co = $sheetObj.ChartObjects($chartId) } catch { $co = $null }
            }
            if ($co) {
                $left = [double]$co.Left
                $top = [double]$co.Top
                ...
                $tb = $sheetObj.Shapes.AddTextbox(1, $left + 8, $top + $height - 24, 260, 18)
                $tb.TextFrame2.TextRange.Text = "Type-B overlays: BG/LBL/REF/ANN"
                $tb.Name = "ANN_$chartId"

                $line = $sheetObj.Shapes.AddLine($left + 8, $top + ($height / 2), $left + $width - 8, $top + ($height / 2))
